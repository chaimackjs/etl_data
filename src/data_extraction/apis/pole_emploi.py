...
def save_jobs_to_file(data, keywords=None, page=1, save_locally=False):
    """
    ...
    filename = f"france_travail_{keywords_str}_{timestamp}_p{page}.json"
    filepath = f"data/raw/france_travail/{filename}"
    
    # Sauvegarder localement uniquement si demandé
    if save_locally:
        ...
        try:
            ...
            logger.info(f"Données sauvegardées localement dans {filepath}")
        except Exception as e:
            logger.error(f"Erreur lors de la sauvegarde locale des données: {e}")
            return None
    
    # Retourner le chemin du fichier même s'il n'est pas sauvegardé localement
    # pour pouvoir l'utiliser pour l'upload vers S3
    return filepath if save_locally else f"data/raw/france_travail/{filename}"
...
def upload_to_s3(file_path, bucket_name='data-lake-brut', data=None):
    """
    Télécharge un fichier ou des données vers S3
    
    Args:
        file_path (str): Chemin du fichier à télécharger ou chemin virtuel pour S3
        bucket_name (str): Nom du bucket S3
        data (dict, optional): Données à télécharger directement sans fichier local
    ...
    try:
        ...
        # Deux modes d'upload : fichier local ou données directes
        if data is not None:
            # Upload direct des données sans fichier local
            logger.info(f"Téléchargement direct des données vers S3: s3://{bucket_name}/{s3_path}")
            
            # Convertir les données en JSON
            json_data = json.dumps(data, ensure_ascii=False).encode('utf-8')
            
            # Uploader directement les données
            ...
            logger.info(f"Données téléchargées avec succès vers S3: s3://{bucket_name}/{s3_path}")
        else:
            ...
            logger.info(f"Téléchargement du fichier vers S3: {file_path} -> s3://{bucket_name}/{s3_path}")
...
def collect_jobs(keywords=None, location=None, distance=None, max_pages=10, upload_s3=True, save_locally=False):
    """
    Collecte les offres d'emploi et les sauvegarde sur S3 et optionnellement en local.
    ...
    Args:
        ...
        save_locally (bool): Si True, sauvegarde également les données en local (désactivé par défaut)
            
    Returns:
        tuple: (total_offres, saved_files) - Nombre total d'offres collectées et liste des fichiers
    """
    ...
    for page in range(1, max_pages + 1):
        ...
        if nb_offres > 0:
            ...
            # Générer un nom de fichier pour S3 (même si on ne sauvegarde pas localement)
            local_file_path = save_jobs_to_file(results, keywords, page, save_locally=save_locally)
            ...
            if local_file_path:
                ...
                if upload_s3:
                    if save_locally:
                        # Upload du fichier local vers S3
                        upload_to_s3(local_file_path, bucket_name)
                    else:
                        # Upload direct des données vers S3 sans fichier local
                        upload_to_s3(local_file_path, bucket_name, data=results)
        else:
