...
et stocke les résultats directement sur S3 par défaut, sans sauvegarde locale.
"""
...
def parse_args():
    ...
    parser.add_argument(
        "--save-locally",
        action="store_true",
        help="Sauvegarder également les données en local (désactivé par défaut)"
    )
    
    return parser.parse_args()

def collect_all_jobs(max_pages=100, delay=1.0, upload_s3=True, force_collect=False, save_locally=False):
    """
    ...
    """
    # S'assurer que les répertoires existent si on sauvegarde localement
    if save_locally:
        os.makedirs("data/raw/france_travail", exist_ok=True)
    
    # Toujours créer le répertoire des logs
    ...
    for page in range(1, max_pages + 1):
        ...
        try:
            ...
            local_file_path = save_jobs_to_file(results, "all", page, save_locally=save_locally)
            
            if local_file_path:
                saved_files.append(local_file_path)
                
                if save_locally:
                    logger.info(f"Données sauvegardées localement dans {local_file_path}")
                ...
                if upload_s3:
                    if save_locally:
                        ...
                        s3_success = upload_to_s3(local_file_path, bucket_name, data=results)
                    
                    if s3_success:
                        logger.info(f"Données téléchargées vers S3: {os.path.basename(local_file_path)}")
                    else:
                        logger.error(f"Erreur lors du téléchargement vers S3: {os.path.basename(local_file_path)}")
...
def main():
    ...
    try:
        ...
        # Vérifier si on doit sauvegarder localement
        if args.save_locally:
            logger.info("Option --save-locally activée: les données seront également sauvegardées en local")
